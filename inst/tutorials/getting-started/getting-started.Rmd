...
tutorial:
  ...
  version: "1.0.20"                 # bump this to force a “fresh” tutorial state
...
The following materials accompany the [Getting Started with dartR](http://dartr.biomatix.org/dartR). 
...
For the more substantive Chapters 3-6, where the analyses build in complexity as they progress, the most productive workflow is to **execute the scripts provided in the RCode Boxes via the RStudio Editor**. This way you are 
...
[Back to Main menu](#custom){.menuButton}
...
```{r eval =FALSE}
df <- read.table("./data/Caretto.dat", header=FALSE)
```

This statement assigns the data in the raw datafile [Caretto.dat]{style = "color: blue;"} to the dataframe `df`. 
...
Download into your working directory, a csv file containing metadata for individuals to be used in later analyses. It is called [Tympo_ind_metadata.csv]{style="color: blue;"}.
...
```{r writecsv, exercise=TRUE, exercise.setup="datainput"}
...
## Chapter 3 {data-progressive=FALSE}
...
In this worked example we will be drawing upon data for the Canberra grassland earless dragon introduced in some detail in eBook Chapter 3. You have your RStudio eBook Project and you have already set the global verbosity to 3 so as to receive full details in the output.
...
## Exercise 3-1: Reading in 1Row Format {data-continue-text="Next Exercise"}

-	Open the file [sample_data_1Row.csv]("http://georges.biomatix.org/storage/app/media/sample_data_1row.csv") in Excel. This is a set of SNP data for *Emydura*, a freshwater turtle,  in 1-row format as would be supplied by DArT.
...
-	Now examine the individual metadata in the file [sample_data_1Row.csv]("http://georges.biomatix.org/storage/app/media/sample_metadata.csv"). Note the two mandatory columns [id]{style="color:blue;"} and [pop]{style="color:blue;"}.
...
-	Add a statement to read the SNP data in to dartR as a genlight object called [gl.1row]{style="color:blue;"}. Check that the data was read in without error.
...
## Exercise 3-2: Reading in SilicoDArT Data {data-continue-text="Next Exercise"}

-	Open the file [sample_data_SilicoDArT.csv]("http://georges.biomatix.org/storage/app/media/sample_data_SilicoDArT.csv") in Excel. This is a set of marker presence/absence data for *Cherax destructor* provided in SilicoDArT format by DArT.
...
-	Add a statement to read the SNP data in to dartR as a genlight object called [gs]{style="color:blue;"}. Check that the data read in without error.
...
## Chapter 4, Session 1 {data-progressive=FALSE}
...
## Chapter 4, Session 2 {data-progressive=FALSE}
...
```{r eval = FALSE}
gl <- gl.load("./data/Tympo_SNP_raw.Rdata")
```
...
We make the decision to delete the populations that has only a single sample from an unknown population. After deleting we check that the desired changes are made using [popNames()]{style="color:blue;"}:
...
The output confirms that the two populations have been deleted, but also issues a warning that monomorphic loci may have arisen with the deletion of the two populations. If this is undesirable, we can run the above command with the parameter [mono.rm=TRUE]{style="color:blue;"}. 
...
gl2 <- gl.reassign.ind(gl, ind.list=c("AA24117", "AA24155", "AA24250", "AA24545"),
       new.pop="Kowen")
```
...
### Tidy up the workspace {data-continue-text="Exercises"}
...
## Exercise 4-3: Using Recode Tables {data-continue-text="Next Exercise"}
...
## Exercise 4-4: Platypus {data-continue-text="Continue"}
...
## Chapter 5 {data-progressive=FALSE}
...
```{r eval = FALSE}
gl <- gl.load("./data/Tympo_SNP_raw.Rdata") # 5276 loci
...
- Using the filtered SilicoDArT Tympo data, run a pca and compare the outcome to the SNP pca. Are they similar? What is different?
